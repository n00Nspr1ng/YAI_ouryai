...
flags.DEFINE_list('tasks', ['yai_pick_and_place_v2'],
...
####### 
yaiconCameraConfig = CameraConfig(
    rgb=True,
    depth=True,
    point_cloud=True,
    mask=False,
...
yaiconObservationConfig = ObservationConfig(
    left_shoulder_camera=yaiconCameraConfig,
    right_shoulder_camera=yaiconCameraConfig,
    overhead_camera=None,
    wrist_camera=yaiconCameraConfig,
    front_camera=yaiconCameraConfig,
...
def run(i, seed_index, lock, task_index, variation_count, results, file_lock, tasks):
    ...
    obs_config = yaiconObservationConfig
            ...
            while attempts > 0:
                try:
                    ...
                    demo, = task_env.get_demos(
                        amount=1,
                        live_demos=True, seed_index=seed_index)
...
def main(argv):
    ...
    ### Multiprocessing - seed 
    seed_index = manager.Value('i', 0) # i - signed integer, start from 0 
    ###
    ...
    processes = [Process(
        target=run, args=(
            i, seed_index, lock, task_index, variation_count, result_dict, file_lock,
            tasks))
