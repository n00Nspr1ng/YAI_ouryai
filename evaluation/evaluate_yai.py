...
from rlbench.tasks import PickAndLift, YaiPickAndPlaceCylinder, YaiPickAndPlaceV2
...
if __name__ == "__main__":
    ...
    elif args.tasks[0] == 'yai_pick_and_place_v2':
        task = env.get_task(YaiPickAndPlaceV2) 
    elif args.tasks[0] == 'pick_and_lift':
    ...
    if save_results:
        ...
        front_rgb.save(os.path.join(os.path.dirname(args.output_file), '0.png'))

    # loads instructions, task_id
    actioner.load_episode(args.tasks[0], variation=0)
    # if args.tasks[0] == 'yai_pick_and_place':
    #     actioner.load_episode("yai_pick_and_place", variation=0) 
    # elif args.tasks[0] == 'yai_pick_and_place_v2':
    #     actioner.load_episode("yai_pick_and_place_v", variation=0) 
    # elif args.tasks[0] == 'pick_and_lift':
    #     task = env.get_task(PickAndLift)
    # else:
    #     raise NotImplementedError 
            ...
            if output.get("trajectory", None) is not None:
                ...
                for action in tqdm(trajectory):
                    ...
                    if save_results:
                        ...
                        outputs['gripper_pose'].append(obs.gripper_pose)
                        front_rgb = Image.fromarray(obs.front_rgb)
                        front_rgb.save(os.path.join(os.path.dirname(args.output_file), f'{step_id+1}.png'))
